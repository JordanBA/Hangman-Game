window.onload = function() {
	var userGuess;
	var words = ["SPURS","ROCKETS","MAVERICKS","HEAT","THUNDER","JAZZ", "HAWKS", "CLIPPERS", "CELTICS"];
	var word = [];
	var positions = [];
	var guessedLetters = [];
	var guesses;
	...
	function newWord(num) {
		...
		if (window.matchMedia("(max-width: 780px) and (orientation: portrait)").matches) {
			...
			// Change directions
			document.querySelector(".red").innerHTML = "Press the arrows to change letter!";
			// Show first letter A
	...
	function answer(letter) {
		...
		// P
		if (words[num] == 'CLIPPERS' || words[num] == 'SPURS') {
